...
function init() {
  ...
  loader.load("/modello.glb",
    (gltf) => {
      ...
      cameraZ *= 1.2;
      camera.position.set(0, size.y * 1, cameraZ);
      camera.lookAt(0, size.y * 0.9, 0);
...
function sendMessage() {
  ...
  let risposta = "Mi dispiace, non ho trovato informazioni.";
  const msgLower = msg.toLowerCase();

  for (let key in knowledge) {
    if (msgLower.includes(key.toLowerCase())) {
      risposta = knowledge[key].risposta;
      break;
    }
  }
